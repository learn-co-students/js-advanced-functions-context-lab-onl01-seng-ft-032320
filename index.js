...
let createEmployeeRecords = (employee) => {
    return employee.map(row => createEmployeeRecord(row));
}
...
let hoursWorkedOnDate = function(soughtDate) {
    let inEvent = this.timeInEvents.find(date => date.date === soughtDate);
    let outEvent = this.timeOutEvents.find(date => date.date === soughtDate);
